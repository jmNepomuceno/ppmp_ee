                ...
                try {
                    ...
                    for(let i = 0; i < response.length; i++){
                        dataSet.push([
                            ...
                            `<span class='view-request-span' id='${response[i].order_by}'> VIEW </span>`,
                        ])
                    }
                    console.log(dataSet)
...
const dataTable_viewRequest = (bioID, sectionName) =>{
    $.ajax({
        url: '../php/view_orderRequest.php',
        method: "POST",
        data : {
            view_bioID : parseInt(bioID)
        },
        dataType : 'JSON',
        success: function(response) {
            console.log(response)
...
$(document).ready(function(){
    ...
    $(document).off('click', '.view-request-span').on('click', '.view-request-span', function() {      
        ...
        // console.log($('.view-request-span').eq(index))
        const bioID = $('.view-request-span').eq(index).attr('id')
        const sectionName = $('.request-section-span').eq(index).attr('id')
        modal_viewRequest.show(
            dataTable_viewRequest(bioID, sectionName)
        )
    });
})